import { useEffect, useState } from 'react';
import { apiFetch } from './api';
import { supabase } from './supabase';

export interface MeResponse {
  user: {
    id: string;
    email: string;
  };
  profile: {
    id: string;
    role: 'user' | 'admin' | 'clinician';
    full_name: string | null;
    avatar_url: string | null;
    phone: string | null;
  } | null;
}

export type MeState =
  | { status: 'loading'; data: null; error: null }
  | { status: 'loaded'; data: MeResponse; error: null }
  | { status: 'error'; data: null; error: string };

export function useMe(): MeState {
  const [state, setState] = useState<MeState>({ status: 'loading', data: null, error: null });

  useEffect(() => {
    let cancelled = false;

    (async () => {
      setState({ status: 'loading', data: null, error: null });

      const { data } = await supabase.auth.getSession();
      const accessToken = data.session?.access_token;
      if (!accessToken) {
        if (!cancelled) setState({ status: 'error', data: null, error: 'Not signed in.' });
        return;
      }

      try {
        const res = await apiFetch('/api/me', { accessToken });
        const json = (await res.json()) as MeResponse & { error?: string };
        if (!res.ok || json.error) {
          if (!cancelled) {
            setState({
              status: 'error',
              data: null,
              error: json.error || 'Failed to load profile.',
            });
          }
          return;
        }

        if (!cancelled) setState({ status: 'loaded', data: json, error: null });
      } catch {
        if (!cancelled) {
          setState({
            status: 'error',
            data: null,
            error: 'Network error. Please try again.',
          });
        }
      }
    })();

    return () => {
      cancelled = true;
    };
  }, []);

  return state;
}

